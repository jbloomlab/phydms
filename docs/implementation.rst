...
A likelihood ratio test can therefore by used to test if adding the pre-determined site-specific diversifying pressure represented by the :math:`\delta_r` values actually improves the description of the gene's evolution.  

We now update the formula in Equation :eq:`dFrxy_dbeta_empirical_phi`. 
We also need to add :eq:`dPrxy_domega2_divpressure`:  

Derivative of :math:`P_{r,xy}` with respect to :math:`\omega2`:  
...
Derivative of :math:`F_{r,xy}` with respect to :math:`\beta`:  

.. math::
   :label: dFrxy_dbeta_empirical_phi_divpressure
   ...
   0 & \mbox{if $\mathcal{A}\left(x\right) = \mathcal{A}\left(y\right)$} \\
   0 & \mbox{if $\mathcal{A}\left(x\right) \ne \mathcal{A}\left(y\right)$ and $\pi_{r,\mathcal{A}\left(x\right)} = \pi_{r,\mathcal{A}\left(y\right)}$} \\
   \frac{F_{r,xy} \left(1 - \frac{F_{r,xy}}{\omega\times\left(1+\omega_{2}\times\delta_{r}\right)} \left(\pi_{r,\operatorname{A}\left(x\right)} / \pi_{r,\operatorname{A}\left(y\right)}\right)^{\beta}\right)}{\beta}
   & \mbox{otherwise,}
   \end{cases}
