...
.. argparse::
    ...
    *ExpCM_<prefsfile>* is an :ref:`ExpCM` with amino-acid preferences taken from the file ``prefsfile``.
    ...
    The preferences file must be a comma-, tab-, or space-delimited file with the first column giving the site and the rest giving the preference for each amino-acid (see detailed example below).
   ...
   \-\-omegabysite
    ...
    For an alternative method to determine site-specific :math:`\omega_r`, please see the ``--random_effects_likelihood`` option.
...
The preferences file should follow the `dms_tools2`_ `preferences file format`_ and be a comma-, tab-, or space-delimited file with the first column giving the site and the rest giving the preference for each amino-acid::
...
+++++++++++++++++++++++++++++++++++++++++++
This file has the suffix ``_omegabycategory.csv``, and is created only if using the ``--random_effects_likelihood`` option.
