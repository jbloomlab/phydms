...
```
Any line starting with `#` will be ignored in any of the file formats.  
...
When testing a particular [diversifying pressure set](#divpressure) or multiple [diversifying pressure sets](#divpressure), there are several controls which are necessary for interpretation of the results. You will proabably want to compare the log-likelihood of the ExpCM model with the diversifying pressure set to the log-likelihood of the ExpCM model without diversifying pressure given the same preferences, alignment, and tree. If there is no signficant change in log-likelihood then you might conclude the diversifying pressure set is not very good. But how do you determine if the change in log-likelihood is signficant? One way is through permutations. By shuffling the values of diversifying pressure across the sites and re-running the analysis you can create a null distribution and determine signifiance of the true diversifying pressure set from this distribution. 

`phydms_test` divpressure is an easy way to run and summarize the results of the analysis described above.  
