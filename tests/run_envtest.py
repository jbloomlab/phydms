"""Tests ``phydms`` on a dataset of HIV env sequences.
...
class TestOnEnvs(unittest.TestCase):
    """
    ...
    def setUp(self):
        ...
        self.bysite_files = ['%s_omegabysite.txt' % model for model in self.all_models if 'YNGKP' not in model]
    ...
    def test_Phydms(self):
        ...
        cmds = ['phydms_comprehensive', self.test_dir, self.alignment, self.prefs, '--ncpus', '-1', '--no-stringencybysite', '--yngkp', '', '--gammarates']
