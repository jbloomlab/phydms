...
def PhyDMSParser():
    ...
    parser.add_argument('--divpressure', type=ExistingFile, help="Specify known diversifying pressure: provide file with column 1 = site, column 2 = divpressure; columns are space delimited; must be a value for every site, with 0 indicating no divpressure.")
...
def PhyDMSTestdivpressureParser():
    ...
    parser.add_argument('divpressure', help='Existing file(s) with diversifying pressure at each site', type=ExistingFile, nargs='+')
    parser.add_argument('--treetopology', help='Fix tree to this Newick topology.', default=None)
    parser.add_argument('--randomizations', help = 'Number diversifying pressure randomizations.', default=0, type=NonNegativeInt)
    parser.add_argument('--ncpus', default=-1, help='Use this many CPUs; -1 means all available.', type=int)
    parser.add_argument('--optimizebrlen', dest='optimizebrlen', action='store_true', help="Optimize tree branchlengths for each diversifying pressure.")
